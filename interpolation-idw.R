...
second.covariate.dt = data.frame(
  ...
  Y = sort(c(first.covariate.dt$Y[index], rnorm(n2))),
...
plot = function(mask, var, formula, ...){
  df = mask
  coordinates(df) = ~ x + y
  gridded(df) = TRUE
  ...
  idw = idw(formula = formula,
             locations = var.df,
             newdata = df)
  idw.output = as.data.frame(idw)
  names(idw.output)[1:3] = c("x", "y", "prediction")
  
  ggplot(data = idw.output, aes(x = x, y = y)) +
    geom_tile(data = idw.output, aes(fill = prediction)) +
    scale_fill_gradient(low = "#FEEBE2", high = "#7A0177") +
    coord_equal()
}

plot(mask.dt,"var2",var2 ~ 1,first.covariate.dt,second.covariate.dt,third.covariate.dt)
